          ...
          {copy.whoQualifies.items.map((item, i) => {
            ...
            return (
              <article key={item.title} className="group bt-card flex h-full flex-col !p-0 overflow-hidden">
                <div className="w-full border-b border-bt-line bg-[rgba(63,144,201,0.08)] px-6 py-5 transition-colors duration-300 group-hover:bg-[rgba(63,144,201,0.14)]">
                  <div className="flex items-start justify-between gap-3">
                    <span className="bt-icon-badge !bg-bt-card">
                      <Icon className="h-5 w-5" aria-hidden />
                    </span>
                    <span className="rounded-full bg-[rgba(63,144,201,0.12)] px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-bt-accent">
                      {item.tag}
                    </span>
                  </div>
                </div>
